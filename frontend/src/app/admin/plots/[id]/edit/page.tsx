...
export default function EditPlotPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchPlotData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/plots/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const plotResponse = await fetch(`http://localhost:8000/plots/${id}`);
      if (!plotResponse.ok) {
      ...
      for (const img of imagesToDelete) {
        try {
          ...
          const deleteResponse = await fetch(`http://localhost:8000/plots/${id}/images/${img.id}`, {
            method: 'DELETE'
          });
